...
def command():
  Parser = argparse.ArgumentParser(prog='SourChimeraHunt.py', usage='./SourChimeraHunt.py [INPUT FASTA] [DATABASE] [-Options]', 
  ...
  Parser.add_argument('INPUT', nargs=1, help='name of input FASTA file containig multiple FASTA sequences, \nplease add directory if file is not in current working directory')
  Parser.add_argument('DATABASE', nargs=1, help='name of database in FASTA format used for reference based chimera detection via vsearch, \nplease add directory if file is not in current working directory')
  Parser.add_argument('-sdb', action='store', default='', dest='secondb', help='name of database in FASTA format used for secondary reference based chimera detection via vsearch\nplease add directory if file is not in current working directory')
  Parser.add_argument('-k', action='store', type=int, default=20, dest='ksize', help='k-mer size for hash calculation\ndefault: 20 nt')
  Parser.add_argument('-l', action='store', type=int, default=1400, dest='length', help='length cut off to filter to short sequences\ndefault: 1400 nt')
...
def time_stamp():                                                                                                               
  ...
  (Yr,Mo,Da,Hr,Mi,Sc,WD,DY,ST) = time.localtime(Time)                                # convert "Unix time" in time units & info 
...
LOG.write('SourChimeraHunt.py\tAlgorithm for detection and extraction of chimeric sequences\n- using sourmash and vsearch\n')
if ARGs.secondb != '':
  LOG.write('- name of inputfile: %s\tname of used databases: %s and \n\n' %(InFileN,DBN,SDBN))
else:
...
if not os.path.isfile(Path):
  ...
  LOG.write('%s\tFile: %s not found. Please check directory!\n' % (time_stamp(),Path))
  sys.exit(0)
else:
...
if ARGs.secondb != '':
  ...
  LOG.write('%s\tFiltering further chimeras using vsearch twice with reference databases: %s and %s\n' % (time_stamp(),DBN,SDBN))
  sys.stdout.write('\nfiltering further chimeras using vsearch twice with reference databases: %s and %s\n' % (DBN,SDBN))
  LOG.write('%s\tFirst vsearch run with reference database: %s \n' % (time_stamp(),DBN))
  os.system('vsearch --uchime_ref %s --db %s --nonchimeras %s/%s_nonchimera_db1.fasta --chimeras %s/%s_misc_chimera_db1.fasta --borderline %s/%s_unknown_db1.fasta' % (ncCCS,ARGs.DATABASE[0],OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta','')))
  os.remove(ncCCS)
  #combining nonchimera  and unknown output of vsearch
  os.system('cat %s/%s_nonchimera_db1.fasta %s/%s_unknown_db1.fasta > %s/%s_nonchimera_in.fasta' % (OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta','')))
  ...
  #secondary analysis
  LOG.write('%s\tSecond vsearch run with reference databases: %s \n' % (time_stamp(),SDBN))
  os.system('vsearch --uchime_ref %s/%s_nonchimera_in.fasta --db %s --nonchimeras %s/%s_nonchimera_db2.fasta --chimeras %s/%s_misc_chimera_db2.fasta --borderline %s/%s_unknown_db2.fasta' % (OUTdir,InFileN.replace('.fasta',''),ARGs.secondb,OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta','')))
  os.remove('%s/%s_nonchimera_in.fasta' % (OUTdir,InFileN.replace('.fasta','')))
  #combining nonchimera  and unknown output of vsearch
  os.system('cat %s/%s_nonchimera_db2.fasta %s/%s_unknown_db2.fasta > %s/%s_nonchimera.fasta' % (OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta','')))
...
#further chimera filtering with vsearch if no second data base is invoked
  ...
  LOG.write('%s\tFiltering for more chimeras using vsearch and reference database: %s\n' % (time_stamp(),DBN))
  sys.stdout.write('\nfiltering further chimeras using vsearch and reference database: %s\n' % DBN)
  os.system('vsearch --uchime_ref %s --db %s --nonchimeras %s/%s_nonchimera1.fasta --chimeras %s/%s_misc_chimera.fasta --borderline %s/%s_unknown.fasta' % (ncCCS,ARGs.DATABASE[0],OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta','')))
  os.remove(ncCCS)

  #combining nonchimera  and unknown output of vsearch
  os.system('cat %s/%s_nonchimera1.fasta %s/%s_unknown.fasta > %s/%s_nonchimera.fasta' % (OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta',''),OUTdir,InFileN.replace('.fasta','')))
